...
class TobiiGlassesController():
	...
	def __send_keepalive_msg__(self, sock, msg):
		while self.streaming:
			sock.sendto(msg.encode(), self.peer)
	...
	def __post_request__(self, api_action, data=None):
		...
		response = urllib.urlopen(req, data.encode())
		res = response.read()
	...
	def __discover_device__(self):
		...
		for i in netifaces.interfaces():
			if netifaces.AF_INET6 in netifaces.ifaddresses(i):
				if "%" in netifaces.ifaddresses(i)[netifaces.AF_INET6][0]['addr']:
					...
					try:
						s6.sendto(b'{"type":"discover"}', (MULTICAST_ADDR, PORT_OUT))
						...
						jdata = json.loads(data.decode('utf_8'))
						log.debug("From: " + address[0] + " " + data.decode('utf_8'))
	...
	def create_project(self, projectname = "DefaultProjectName"):
		...
		if project_id is None:
			data = {'pr_info' : {'CreationDate': self.project_creation_date, 'EagleId':  str(uuid.uuid5(uuid.NAMESPACE_DNS, str(projectname.encode('utf-8')))), 'Name': projectname}}
	...
	def create_participant(self, project_id, participant_name = "DefaultUser", participant_notes = ""):
		...
		if participant_id is None:
			data = {'pa_project': project_id, 'pa_info': {'EagleId': str(uuid.uuid5(uuid.NAMESPACE_DNS, str(self.participant_name.encode('utf-8')))), 'Name': self.participant_name, 'Notes': participant_notes}}
